...
class UserBehavior(TaskSet):
    ...
    @task(1)
    def addRoom(self):
        response =  self.client.post("/offer/orders",json={
        ...
        if int(id) > 0:
            #update 
            self.client.put("/offer/orders/"+str(id),json={
            "offer_id":self.randomInt(),
            "order_id":self.randomInt()
            },headers=headers,name="/offer/orders/:id")
            
            #delete 
            self.client.delete("/offer/orders/"+str(id),headers=headers,name="/offer/orders/:id")
