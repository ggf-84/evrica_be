...
class UserBehavior(TaskSet):
    ...
    @task(1)
    def addRoom(self):
        response =  self.client.post("/vat",json={
        ...
        if int(id) > 0:
            #update 
            rsp = self.client.put("/vat/"+str(id),json={
               "value":self.randomInt(),
           
            },headers=headers,name="/vat/:id")

            
        #delete 
        self.client.delete("/vat/"+str(id),headers=headers,name="/vat/:id")
