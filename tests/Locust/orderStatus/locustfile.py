...
class UserBehavior(TaskSet):
    ...
    @task(1)
    def addRoom(self):
        response =  self.client.post("/order/status",json={
        ...
        if int(id) > 0:
            #update 
            self.client.put("/order/status/"+str(id),json={
               "title":self.randomString(),
              
        
        },headers=headers,name="/order/status/:id")
            
        #delete 
        self.client.delete("/order/status/"+str(id),headers=headers,name="/order/status/:id")

    @task(2)
    def getAll(self):
        response = self.client.get("/order/status",headers=headers)
