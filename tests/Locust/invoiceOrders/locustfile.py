...
class UserBehavior(TaskSet):
    ...
    @task()
    def addRoom(self):
        response =  self.client.post("/invoice/orders",json={"invoice_id":self.randomInt(),"order_id":self.randomInt()}, headers=headers)

        id = json.loads(response.content)["id"]
        
        if int(id) > 0:
            #update contact categories
            self.client.put("/invoice/orders/"+str(id),json={"invoice_id":self.randomInt(),"order_id":self.randomInt()},headers=headers,name="/offers/:id")
            
            #delete contact categories
            self.client.delete("/invoice/orders/"+str(id),headers=headers,name="/offers/:id")

    #Get all event types
    @task()
    def getById(self):
        response = self.client.get("/invoice/"+str(getId)+"/orders",headers=headers)
