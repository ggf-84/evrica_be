...
class UserBehavior(TaskSet):
    ...
    @task(1)
    def addRoom(self):
        response =  self.client.post("/measure",json={
        ...
        if int(id) > 0:
            #update 
            self.client.put("/measure/"+str(id),json={
            "title":self.randomString()
            },headers=headers,name="/measure/:id")
            
            #delete 
            self.client.delete("/measure/"+str(id),headers=headers,name="/measure/:id")
